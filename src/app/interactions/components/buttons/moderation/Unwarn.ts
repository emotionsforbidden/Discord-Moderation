            ...
            return await this.responses.replyWithMessage(ctx, {
                embeds: [
                    this.responses
                        .getEmbedTemplate(ctx.member.displayAvatarURL())
                        .setTitle('Unwarn member')
                        .setDescription(`Member **doesn't have warnings** yet`)
                ],
                components: []
            });
        }

        await this.moderation.unwarn({
            target,
            doc,
            executor: ctx.member
        });
